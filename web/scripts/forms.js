...
function validateNumericField(myField) {
	var txt = myField.value;
	var regex = /[A-Za-z]/; //comprende tutte le lettere esclusi i numeri
	if (txt.charAt(txt.length - 1).match(regex)) //se l'ultimo carattere inserito è una lettera -> lo cancello
		//txt = txt.slice(0, txt.length - 1);
		myField.value = txt.slice(0, txt.length - 1);
}
